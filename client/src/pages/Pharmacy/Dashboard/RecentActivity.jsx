...
const ActivitySkeleton = () => (
  <div className="space-y-4">
    {Array.from({ length: activityConfig.length }).map((_, idx) => (
...
export default function RecentActivity() {
  const {
    ...
    queryKey: ['pharmacy', 'dashboard', 'recent-activity'],
    queryFn: async () => {
      ...
      {isLoading && !data ? (
        ...
        <div className="space-y-4">
          {activityConfig.map((activity) => {
            ...
            const timestamp = activity.key === 'latestDrug' ? record?.createdAt : record?.deliveredAt;
            ...
            return (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-600 mt-1">{activity.formatDescription(record)}</p>
                  <p className="text-xs text-gray-400 mt-1">{formatRelativeTime(timestamp)}</p>
                </div>
              </div>
            );
          })}
